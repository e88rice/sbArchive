...
checkAll.addEventListener('change', function (e) {
    let boardId = document.querySelectorAll('input[name=signboardId]');
...
// 타겟 삭제
function targetDelete(signboardId){
    if(confirm("정말 삭제하시겠습니까?")){
        let newForm = document.createElement('form');

        newForm.name = 'targetDelete';
        newForm.method = 'post';
        newForm.action = '/my/deleteTargetSignBoard';

        let input1 = document.createElement('input');

        // set attribute (input)
        input1.setAttribute("type", "hidden");
        input1.setAttribute("name", "signboardId");
        input1.setAttribute("value", signboardId);
